...
public class Task3_To_Win : MonoBehaviour, ITaskHandler
{
    public int maxRound = 7;
    public int WinScore = 70;
    ...
    private ScoreCalculator_3 scoreCalculator;

    void Start()
    {
        scoreCalculator = GetComponent<ScoreCalculator_3>();
        if (resultPanel != null)
    ...
    public void OnPlayerMessage(string playerMessage)
    {
        ...
        int roundScore = scoreCalculator.CalculateFrontendScore(playerMessage);
    ...
    void UpdateUI()
    {
        ...
        if (roundText != null)
        {
            roundText.text = $"第{currentRound}/{maxRound}轮对话";
        }
    }
    ...
    IEnumerator EndGameWithAnimation()
    {
        ...
        if (resultText != null)
            resultText.text = isWin ? "面试通过！获得Offer！" : "面试失败，继续努力";
